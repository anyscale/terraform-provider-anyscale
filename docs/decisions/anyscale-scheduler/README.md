...
14. Order of `scheduling_rules` and `flavors` round-trips exactly; a reordered config produces a
    ...
    **The expectation must be the literal declared order, not a value re-derived from Go
    structures.** A helper that rebuilds the expected document by marshaling Go maps cannot
    represent ordering at all, so a test written on one passes against a build that reorders —
    the same placebo shape as `jsonencode()` in criterion 15, and for the same reason: the
    expectation is generated by the machinery under test. The known instance is `readConfigAuto`;
    the rule outlives the helper's name. Hand-write both orderings.

    **What keeps the convergence step honest is one non-obvious interaction in the test server, and
    it is worth knowing before anyone tidies it.** The criterion ends by injecting a reordered
    document as out-of-band drift and asserting that the corrective apply converges to an empty
    plan. That only proves something because the mock echoes the *posted bytes* back on the
    following read: `SetReadConfig` disables the auto-generated read document but deliberately
    leaves the echo mode enabled, so the injected drift serves the plan and the corrective apply
    then overwrites it. A build that reordered on the write path would echo its reordered bytes and
    the final plan would diff. Turn the echo off — or "fix" `SetReadConfig` to clear it, which is
    what it looks like it should do — and the last step asserts a fixture agreeing with itself.
