...
subcategory: ""
description: |-
  Manages a user's RBAC role on an Anyscale cloud (base_role plus optional deny_roles) - a newer, richer role model than the legacy owner/write/readonly permission level, and currently the only way to grant project_viewer, compute_config_viewer, or workload_operator. These roles are not visible or settable in the Anyscale console today; this resource is the only interface to them.
