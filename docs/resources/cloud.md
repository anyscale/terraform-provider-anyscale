...
- `auto_add_user` (Boolean) Whether to automatically add users to this cloud. Note: while enabled, destroying an anyscale_cloud_user_role assignment on this cloud will fail, since the backend refuses to leave any user without cloud access, see that resource documentation for the interaction.
