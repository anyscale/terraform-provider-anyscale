...
- `base_role` (String) The collaborator's base role in the organization (`owner` or `collaborator`). `permission_level` above remains the field you set to change it; the two always agree since the backend derives `base_role` from `permission_level` on every read (not the reverse).
