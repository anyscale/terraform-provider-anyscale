...
- `cloud_id` (String) The cloud ID for this project. If a later refresh finds the backend reports no associated cloud (an inconsistent state), this reads as null rather than an empty string. To reference a cloud by name instead of by id, look it up with the [`anyscale_cloud` data source](../data-sources/cloud.md) (`cloud_id = data.anyscale_cloud.example.id`).
