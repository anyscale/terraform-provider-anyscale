...
- `cloud_id` (String) The cloud ID for this project. Either `cloud_id` or `cloud_name` must be specified. If configured with cloud_id and a later refresh finds the backend reports no associated cloud (an inconsistent state), this reads as null rather than an empty string.
