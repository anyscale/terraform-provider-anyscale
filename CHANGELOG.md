...
- resource/anyscale_cloud: `file_storage` changes (including adding or removing a `persistent_volume_claim`) now update the cloud in place instead of forcing it to be destroyed and recreated, and drift on `file_storage` is now surfaced during `terraform plan` instead of only on re-import.
- resource/anyscale_cloud_resource: `file_storage` changes (including adding or removing a `persistent_volume_claim`) now update the cloud in place instead of forcing it to be destroyed and recreated, and drift on `file_storage` is now surfaced during `terraform plan` instead of only on re-import.

## [0.27.0] - 2026-08-11

### New Resources

- resource/anyscale_cloud_iam_mapping: manage the ordered dataplane IAM mapping rules for a cloud deployment, including destroy-as-revert.

### New Data Sources

- data-source/anyscale_cloud_iam_mapping: read a cloud deployment's dataplane IAM mapping rules.
...
- resource/anyscale_organization_user: `permission_level`, `base_role`, and `additional_roles` are removed. Manage a user's organization-level role with the new anyscale_organization_user_role resource instead; since `permission_level` was Required, this breaks configuration and not only state, so update your configuration and re-import.
...
- resource/anyscale_cloud: `file_storage.mount_targets` changes from a block to a list-of-objects attribute; rewrite `mount_targets { address = "..." zone = "..." }` as `mount_targets = [{ address = "...", zone = "..." }]`. The value is now Computed and self-heals the same way as the memorydb/memorystore fix for clouds imported between v0.15.2 and v0.16.0 inclusive; clouds imported with v0.16.1 or later keep their null value until re-imported. Existing state auto-migrates to the new schema on your next plan alongside the kubernetes_config change below; no manual state surgery is needed.
- resource/anyscale_cloud_resource: `file_storage.mount_targets` changes from a block to a list-of-objects attribute; rewrite `mount_targets { address = "..." zone = "..." }` as `mount_targets = [{ address = "...", zone = "..." }]`. The value is now Computed and self-heals the same way as the memorydb/memorystore fix for clouds imported between v0.15.2 and v0.16.0 inclusive; clouds imported with v0.16.1 or later keep their null value until re-imported. Existing state auto-migrates to the new schema on your next plan alongside the kubernetes_config change below; no manual state surgery is needed.
- resource/anyscale_cloud: `kubernetes_config.namespace`, `ingress_host`, `cluster_name`, `context`, and `kubeconfig_path` are removed; they were deprecated since v0.1.2 and already had no effect (never sent to the Anyscale API). To migrate, delete these five attributes from your configuration — `terraform plan`/`validate` errors with "Unsupported argument" if any remain. Existing state auto-migrates to the new schema on your next plan; no manual state surgery is needed, including for clouds that never set any of the five fields.
- resource/anyscale_cloud_resource: `kubernetes_config.namespace`, `ingress_host`, `cluster_name`, `context`, and `kubeconfig_path` are removed; they were deprecated since v0.1.2 and already had no effect (never sent to the Anyscale API). To migrate, delete these five attributes from your configuration — `terraform plan`/`validate` errors with "Unsupported argument" if any remain. Existing state auto-migrates to the new schema on your next plan; no manual state surgery is needed, including for clouds that never set any of the five fields.
