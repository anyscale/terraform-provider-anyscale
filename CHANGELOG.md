...
- resource/anyscale_project: the delete-time 403 retry for a recently-created project (introduced in v0.5.1) now uses a capped-exponential backoff up to 60 seconds total, instead of a fixed 3 attempts over about 6 seconds, since real-world backend permission-check lag can exceed the original window.
