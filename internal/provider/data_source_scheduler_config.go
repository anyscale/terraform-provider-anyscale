	...
	resp.Schema = schema.Schema{
		...
		Attributes: map[string]schema.Attribute{
			...
			"recycle_policy": schema.SingleNestedAttribute{
				Computed:            true,
				MarkdownDescription: "When the scheduler retires and replaces the machines it manages. Reported as stored; no Anyscale component acts on it yet, so a policy here does not describe scheduling behavior in effect. `null` if the config sets no recycle policy.",
				Attributes: map[string]schema.Attribute{
