...
Every read re-fetches fresh: there is no caching, renewal, or automatic refresh between separate reads (this resource implements Open only, with no Renew or Close). ` + "`workload_service_url_auth`" + ` is only non-null while the System Cluster is currently ` + "`Running`" + ` - it is ` + "`null`" + ` (accompanied by a warning diagnostic explaining why) if the cloud has no System Cluster yet, or if one exists but has not reached ` + "`Running`" + `.
