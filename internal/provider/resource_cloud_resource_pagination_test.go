...
// TestFindDefaultCloudResource_PagesBeyondFirstPage confirms the default-resource
// check considers every page before deciding whether a cloud has a single default.
func TestFindDefaultCloudResource_PagesBeyondFirstPage(t *testing.T) {
	requestCount := 0
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		requestCount++
		w.WriteHeader(http.StatusOK)
		...
		if requestCount == 1 {
			_, _ = fmt.Fprint(w, `{
				"results": [{"name": "resource-1", "is_default": false}],
				"metadata": {"total": 2, "next_paging_token": "page2"}
			}`)
			return
		}
		...
		_, _ = fmt.Fprint(w, `{
			"results": [{"name": "resource-2", "is_default": true}],
			"metadata": {"total": 2, "next_paging_token": null}
		}`)
	}))
	defer server.Close()
	...
	// Two total results across both pages, only one default: findDefaultCloudResource
	// only returns non-nil when there is EXACTLY ONE result overall, so this
	// correctly returns nil here - the point is that it must consider both
	// pages to reach that conclusion, not stop at page 1's single result.
	result, err := r.findDefaultCloudResource(context.Background(), "cloud-id")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if result != nil {
		t.Errorf("expected nil (2 results across pages, not a single default), got %+v", result)
	}
	if requestCount != 2 {
		t.Errorf("expected 2 requests (one per page), got %d", requestCount)
	}
