...
func TestCloudUserRoleDelete_UnrepairableStateGetsExplanatoryDiagnosticNotRawPassthrough(t *testing.T) {
	...
	// PLACEBO GUARD, not a redundant check: the assertion below only proves
	// the diagnostic CONTAINS the explanatory phrases, which says nothing
	// about whether the mock's own raw error text already contains them too -
	// a separate fact. If this fixture ever drifted to include wording like
	// "state rm", a regressed raw-passthrough implementation would satisfy
	// that assertion by accident, and this test would become a silent
	// placebo without anyone touching its logic. This precondition fails
	// loudly instead, the moment the fixture starts overlapping with what
	// only the real wrapping logic should add.
