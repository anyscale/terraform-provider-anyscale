...
locals {
  ...
  gpu_node_pools = flatten([
    for gpu_type in keys(var.gpu_instance_configs) : [
      merge(local.base_node_pool,
        merge(var.gpu_instance_configs[gpu_type].instance, {
  ...
  node_pools_labels = merge(
    { all = {} },
    {
      for gpu_type in keys(var.gpu_instance_configs) : "ondemand-gpu-${lower(gpu_type)}" => var.gpu_instance_configs[gpu_type].node_labels
    },
    {
      for gpu_type in keys(var.gpu_instance_configs) : "spot-gpu-${lower(gpu_type)}" => var.gpu_instance_configs[gpu_type].node_labels
    }
  )
